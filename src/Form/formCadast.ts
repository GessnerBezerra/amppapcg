...
let formCadast = document.querySelector("#cadastro-container") as HTMLDivElement;
...
export function salvarNoForm(dadosdriver: Driver[]) {
  if (dadosdriver.length > 0) {
    for (const indice in dadosdriver) {
      let indcString: string | undefined = dadosdriver[indice].id;
      ...
      if (indcNumber % 2 == 0) {
        sectionCard.setAttribute(
          "style",
          "background: linear-gradient(to right,rgba(144, 203, 44, 0.8),rgba(144, 203, 44, 0.6))"
        );
      }
      if (indcNumber % 2 != 0) {
        sectionCard.setAttribute(
          "style",
          "background: linear-gradient(to right,rgba(44 203 190 / 80%),rgba(73, 44, 203, 0.6))"
        );
      }
